<script>
    ...
    import { url, activities, pageCount, type, experience, subject } from './store.module.js'
...
<nav class="w-full mb-12 pagination" aria-label="pagination">
    ...
    {#if offset > 0}
        <li class="ml-2"><a on:click|preventDefault={changePage} class="p-2" href="?page={page - 1}" data-page={page - 1}><span aria-hidden="true">&larr;</span><span class="sr-only">previous</span></a></li>
    {/if}
    {#each pages as p, i}
    <li class="ml-2"><a on:click|preventDefault={changePage} class="{offset === p ? "p-2 text-white bg-black" : "p-2"}" href="?page={i + 1}" data-page={i + 1}>{i + 1}</a></li>
    {/each}
    {#if $activities.length > offset + 10}
        <li class="ml-2"><a on:click|preventDefault={changePage} class="p-2" href="?page={page + 1}" data-page={page + 1}><span class="sr-only">next</span><span aria-hidden="true">&rarr;</span></a></li>
    {/if}
    </ul>
</nav>
{/if}
